...
class Averager:
    """
    变量count和total，都保存在类实例avg_cls中
    每次对类实例的调用，都会调用__call__方法计算平均值，因为是同一个实例，所以count和total的值都会存储在实例中。
    闭包也是类似的效果，变量count和total也是存储在外层函数的上下文中
    所以每次对内层函数调用，外层函数的变量不会被销毁，而是一直保持初始值（未调用内层函数时）
    或上次调用的结果（每次调用内层函数后，对count和total的修改都会被存储）。
    """
