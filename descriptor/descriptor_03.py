...
if __name__ == '__main__':
    ...
    《Python Cookbook》8.10 让属性具有惰性求值的能力，即是用非覆盖型描述符，让实例属性的读取实现惰性求值，以提高执行性能。
    核心的逻辑就是在描述符执行__get__方法时，同时对同名实例属性进行赋值，覆盖掉描述符自身。
    那么在下次读取时，直接读取的便是实例自身的属性，不会再调用描述符的__get__进行运算，以达到提高性能的目的。
    '''
